<template>
  <div>
        ...
        <div class="item-list-wrap">
          <div class="cart-item">
            ...
            <ul class="cart-item-list">
              <li v-for="(val, index) of cartList" :key="index">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    ...
                    <a
                      ...
                      @click="editCart(val, val)"
                      :class="{ check: val.checked }"
                    >
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="editCart('min', val)">-</a>
                        <span class="select-ipt">{{ val.productNum }}</span>
                        <a class="input-add" @click="editCart('add', val)">+</a>
                      </div>
                    </div>
                  </div>
                </div>
                <div class="cart-tab-4">
                  <div class="item-price-total">
                    {{ (val.productNum * val.scalePrice) | myCurrency }}
                  </div>
                </div>
          ...
          <div class="cart-foot-inner">
            ...
            <div class="cart-foot-r">
              <div class="item-total">
                Item total:
                <span class="total-price">{{ totalPrice | myCurrency }}</span>
              </div>
              <div class="btn-wrap">
                <!-- 按钮添加btn--dis则按钮禁用,商品全部未选中时禁用 -->
                <a
                  class="btn btn--red"
                  @click="checkOut"
                  :class="{ 'btn--dis': totalPrice == 0 }"
                  >Checkout</a
                >
              </div>
            </div>
    ...
    <modal :mdShow="del" @update="update">
      ...
      <div slot="btn">
        <a class="btn btn--m" @click="delProduct">确定</a>
...
export default {
  ...
  computed: {
    totalPrice() {
      ...
      this.cartList.forEach((item) => {
        if (item.checked) {
          money += item.scalePrice * item.productNum;
        }
      });
      return money;
    },
  ...
  methods: {
    ...
    toggleCheckAll() {
      ...
      axios.post("/users/checkAll", { checkAll: this.checkAll }).then((res) => {
        // console.log(res);
      });
    },
    editCart(flag, val) {
    ...
    delProduct() {
      ...
      axios
        .get("/users/delProduct", { params: { productId: this.productId } })
        .then((res) => {
          //   console.log(res);
