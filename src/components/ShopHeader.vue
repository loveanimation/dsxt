<template>
  <header class="header">
    ...
    <div class="navbar">
      ...
      <div class="navbar-right-container" style="display: flex">
        <div class="navbar-menu-container">
          ...
          <a v-if="nickName" href="javascript:void(0)" class="navbar-link">{{
            nickName
          }}</a>
          <a
            v-else
    ...
    <div
      ...
      v-bind:class="{ 'md-show': show_modal }"
    >
      <div class="md-modal-inner">
        <div class="md-top">
          <div class="md-title">Login in</div>
          <button class="md-close" @click="show_modal = false">Close</button>
        </div>
...
export default {
  ...
  methods: {
    ...
    login() {
      ...
      axios.post("users/login", { userName, userPwd }).then((res) => {
        let data = res.data;
        if ((data.status = "0")) {
